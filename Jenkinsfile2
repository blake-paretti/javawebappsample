...
node {
 withEnv(['AZURE_SUBSCRIPTION_ID=e731c36d-5d51-465d-996b-61c1d1bcec2b',
    ...
    stage('deploy') {
      def webAppResourceGroup = 'PipelinePracticeVM_group'
      def webAppName = 'TestAzurePipeline'
      def acrName = 'PipelineContainerRegistry'
      def imageName = 'pcappservicelinuxrg'
      ...
      withCredentials([usernamePassword(credentialsId: 'AzureServicePrincipal', passwordVariable: 'Bpa10252648!', usernameVariable: 'blake.paretti@perficient.com')]) {
        // login Azure
        sh '''
